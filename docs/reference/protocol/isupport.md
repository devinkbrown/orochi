...
| `CHANMODES` | `beIZ,k,lfj,imnstCTNMSgWOAVU` | Four channel-mode classes: list, param-always, param-on-set, flag. | Static token from `chanmodes_token`. `W` (NOWHISPER), `O` (oper-only), `A` (admin-only), `V` (NOCOMICDATA), and `U` (OPMODERATE) are live flag modes; see `modes.md`. | `src/proto/protocol_inventory.zig` (`chanmodes_token`) |
