...
| `NOTE` | `standard_replies.note` / `standard_replies_emit.note`, plus Event Spine `NOTE EVENT`. | `NOTE <command> <code> [context...] :<description>` or `:server NOTE EVENT <CATEGORY> :...` | Event Spine operator notices, session/media/tegami/filter notes; cataloged builder support. | `src/proto/standard_replies.zig:210`, `src/proto/standard_replies_emit.zig:136`, `src/daemon/server.zig:10009`, `src/daemon/event_spine.zig:292` |
