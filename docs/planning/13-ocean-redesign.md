...
- Next.js 16 (React 19, App Router), TS5, Tailwind 4, Zustand 5, Vitest+Playwright,
  static export to `out/`. **pnpm only.**
- Transport: WebSocket IRCv3 (`lib/irc/client.ts`) — SASL (SCRAM-SHA-512/256,
  SESSION-TOKEN, PLAIN), full CAP negotiation incl. `draft/chathistory,reply,react,
  typing,message-editing,message-redaction,multiline,read-marker`, plus
  `ophion/ladon-media`, `ophion/prop-notify`, `ophion/session-sync`.
- Media: LADON over IRC (`MEDIAFRAME`/`MCHUNK`) + VEIL (P-256 ECDH + AES-256-GCM),
  Opcodec WASM (`lib/ladon-media/`).
- 138 components; ocean-depth theme (3 palettes); 40+ modals (modal fatigue).

## #16 direction: "mesh-native collaboration interface"
Center the real differentiators (open IRCv3 + E2EE LADON + IRCX role/permission
model) instead of a Discord-clone 3-column + modal cabinet. Sans+mono pairing,
bioluminescent activity accents, HLC-aware adaptive dark.
Prioritized surfaces: dynamic layout (collapse serverbar <3 servers; **presence
ribbon** top; **thread sidebar** right replacing ThreadPanel modal), threaded
message tree, reactions with reactor avatar stacks, global `/` Spotlight command
...
ForumBrowser, RoleMatrixEditor, voice PIP.

This is planning only. Ocean lives in /home/kain/onyx (proprietary; never GitHub).
