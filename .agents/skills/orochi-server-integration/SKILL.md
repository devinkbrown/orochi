---
name: orochi-server-integration
description: Integrate completed Orochi modules into daemon lifecycle and server.zig transactionally. Use when wiring session, Helix, mesh, Event Spine, delivery, persistence, capability, reconnect, or shutdown behavior into the live server.
---

# Integrate the live daemon

The `orochi-server-integrator` role holds a permanent named-role reservation on `src/daemon/server.zig` across all assignments. No other role may receive that file. Do not absorb leaf-module work that another agent owns.
