...
//! Undertow mesh coordinator: partial-view membership + eager/lazy broadcast +
//! witnessed Ripple failure detection, composed into one transport-agnostic
//! state machine.
//!
//! Each of the three pieces is an independently-tested pure driver
//! (`gossip_views.Views`, `gossip_views.Broadcast`, `ripple.Ripple`). This module is
...
//!   * witnessed Ripple decides *which* peers are alive, and a quorum of witnesses
//!     is required before any node is declared dead (no single accuser).
//!
//! The feedback loop is the real work here:
//!   - when the overlay adds an active peer, Ripple is told to start probing it;
//!   - when Ripple declares a peer dead, the overlay disconnects it, and Broadcast
...
const ripple = @import("ripple.zig");
...
pub const State = ripple.State;

pub const Error = gossip.Error || ripple.Error || std.mem.Allocator.Error;

pub const Config = struct {
    views: gossip.Config = .{},
    broadcaster: gossip.BroadcastConfig = .{},
    ripple: ripple.Config = .{},
...
pub const Wire = union(enum) {
    ...
    membership: struct {
        ...
        state: State,
        incarnation: u64,
        witnesses: ripple.WitnessSnapshot,
    },
};
...
pub const Mesh = struct {
    ...
    pub fn init(self: *Mesh, allocator: std.mem.Allocator, self_id: NodeId, cfg: Config) Error!void {
        ...
        self.ripple = try ripple.Ripple.init(allocator, self_id, cfg.ripple);
        errdefer self.ripple.deinit();
    ...
    pub fn status(self: *const Mesh, node: NodeId) State {
        return self.ripple.status(node);
    }
    ...
    /// Advance time-based behaviour: Ripple probing/suspicion, Broadcast GRAFT
    /// retries, and death-driven overlay eviction.
    pub fn tick(self: *Mesh, now_ms: i64) Error![]Envelope {
        var out: std.ArrayList(Envelope) = .empty;
        errdefer self.freeEnvelopeList(&out);

        const saz_actions = try self.ripple.tick(now_ms, &self.rng);
        defer self.ripple.freeActions(saz_actions);
        try self.translateRipple(&out, saz_actions);
    ...
    pub fn recv(self: *Mesh, from: NodeId, msg: Wire, now_ms: i64) Error!RecvResult {
        ...
        switch (msg) {
            ...
            .ack => try self.ripple.onAck(from),
            .ping_req => |pr| {
                const actions = try self.ripple.onPingReq(from, pr.target, now_ms);
                defer self.ripple.freeActions(actions);
                try self.translateRipple(&out, actions);
            },
            .membership => |m| try self.ripple.onMembershipDelta(.{
                .node = m.node,
                .state = m.state,
    ...
    ///   1. register every active-view peer as a Ripple member so it gets probed
    ///      (Ripple treats *unknown* nodes as dead, so this must run before any
    ///      death check, and it cannot resurrect a genuinely-dead member);
    ///   2. evict any active peer Ripple now reports dead, so Broadcast drops it.
    ...
    fn reconcile(self: *Mesh, out: *std.ArrayList(Envelope), now_ms: i64) Error!void {
        // Snapshot the active view before touching Ripple/views; overflow simply
        ...
        for (live_buf[0..live_len]) |peer| {
            ...
            if (!self.ripple.isMember(peer)) {
                try self.ripple.onMembershipDelta(.{ .node = peer, .state = .alive }, now_ms);
            }
        }
        ...
        for (live_buf[0..live_len]) |peer| {
            if (self.ripple.status(peer) == .dead and dead_len < dead_buf.len) {
    ...
    /// Ripple `Declare`s are membership gossip with no fixed recipient; fan them
    /// out over the active view so the epidemic spreads.
    fn translateRipple(self: *Mesh, out: *std.ArrayList(Envelope), actions: []const ripple.Action) Error!void {
        for (actions) |a| {
            switch (a) {
...
// every node, and converge on a peer's death through witnessed Ripple.
// ---------------------------------------------------------------------------
        ...
        fn init(self: *Self, allocator: std.mem.Allocator) !void {
            ...
            for (&self.nodes, 0..) |*node, i| {
                try node.init(allocator, @intCast(i + 1), .{
                    ...
                    .ripple = .{ .period_ms = 100, .suspect_timeout_ms = 300, .quorum = 2 },
...
test "witnessed Ripple converges on a dead peer and evicts it from the overlay" {
    const allocator = testing.allocator;
    const N = 4;
    var sim: Sim(N) = undefined;
    try sim.init(allocator);
...
test "Config.applyToml composite delegates to views/broadcaster/ripple and rng_seed" {
    ...
    try std.testing.expectEqual(@as(usize, 7), cfg.ripple.k);
