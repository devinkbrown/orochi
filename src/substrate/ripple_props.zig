...
//! Deterministic property tests for the RIPPLE membership state machine.
const std = @import("std");
const ripple = @import("ripple.zig");

const Config = ripple.Config;
const Incarnation = ripple.Incarnation;
const Member = ripple.Member;
const MemberState = ripple.MemberState;
const Membership = ripple.Membership;
const NodeId = ripple.NodeId;
const Reaped = ripple.Reaped;
...
fn expectProbeInvariants(table: *const Membership, probe: ripple.Probe) !void {
    if (!probe.active) {
    ...
    try std.testing.expect(probe.indirect_len <= ripple.max_tracked_witnesses);
...
test "property: witness tracking respects fixed maximum without overflow" {
    ...
    for (0..16) |iteration| {
        ...
        var i: u16 = 0;
        while (i < ripple.max_tracked_witnesses + 8) : (i += 1) {
            ...
            try std.testing.expect(count <= ripple.max_tracked_witnesses);
            try expectState(&table, target, .suspect);
        }

        try std.testing.expectEqual(
            @as(u8, ripple.max_tracked_witnesses),
            (try expectMember(&table, target)).witnessCount(),
        );

        i = 0;
        while (i < ripple.max_tracked_witnesses) : (i += 1) {
            const duplicate = 1_000_000 + @as(NodeId, @intCast(random.uintLessThan(u16, ripple.max_tracked_witnesses)));
