...
//! Pure UNDERTOW S2S link-session orchestrator.
//!
//! `peer_link` owns link sequencing, acknowledgements, and credit. This layer
//! frames convergent UNDERTOW work over that link: initial channel burst, live
...
pub const Config = struct {
    ...
    ripple_config: gossip_round.RippleConfig = .{},
    ...
    /// (gossip_config, ripple_config) are applied by the orchestrator separately.
...
pub const LinkSession = struct {
    ...
    /// Ripple/repair immediately target it. `state` is a FRESH empty replica — the
