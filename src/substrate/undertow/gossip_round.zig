...
//! Pure UNDERTOW gossip round driver: no sockets, timers, or daemon coupling.
...
pub const DeltaDot = concord.Dot;
const max_tracked_witnesses = 16;

pub const RippleConfig = struct {
    suspicion_timeout_ms: i64 = 3_000,
    witness_quorum: u8 = 2,

    fn sanitized(self: RippleConfig) RippleConfig {
        var c = self;
...
pub const Membership = struct {
    ...
    fn init(allocator: std.mem.Allocator, self_id: NodeId, cfg: RippleConfig) Membership {
        return .{
            .allocator = allocator,
            .cfg = cfg.sanitized(),
            .self_id = self_id,
...
pub const GossipRound = struct {
    ...
    pub fn init(
        ...
        return .{
            ...
            .self_id = self_id,
            .view = view,
            .membership = Membership.init(allocator, self_id, ripple_cfg),
        };
    }
...
test "witness_quorum floors to 2 so a single witness never buries a peer" {
    ...
    try testing.expectEqual(@as(u8, 2), (RippleConfig{ .witness_quorum = 1 }).sanitized().witness_quorum);
    try testing.expectEqual(@as(u8, 2), (RippleConfig{ .witness_quorum = 0 }).sanitized().witness_quorum);
