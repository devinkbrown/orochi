...
| `allowed_caps` | array of strings | `["reply","log","time"]` | `reply`, `log`, `time`, `rand`, `store`, `lookup`, `hooks` | Hostcall capability classes a plugin may receive after manifest negotiation. Registry trust tiers still cap grants: unlisted may receive `reply`/`log`/`time`/`rand`, listed may also receive `store`/`lookup`, and verified is required for `hooks`. Omitted keeps the minimal control-plane set; `[]` denies every hostcall. |
