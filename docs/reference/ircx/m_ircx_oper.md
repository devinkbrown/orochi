# m_ircx_oper — Provides IRCX operator tools: GAG mode (+g), OPFORCE commands
> Source-verified module reference for `m_ircx_oper`.

## Overview
`m_ircx_oper` is a C module implemented in `modules/m_ircx_oper.c`. Provides IRCX operator tools: GAG mode (+g), OPFORCE commands.
