# Invented technologies, build, and roadmap
...
This document records the planning-phase invention catalog, build and test strategy, roadmap, and risk register for Orochi.

## Grounding

Orochi is a clean-slate Zig rewrite of daemon, substrate, and crypto, with ophion/libop/opssl as reference only, while preserving IRCv3, IRCX, SASL, services, WebSocket, CHATHISTORY, voice/video, and replacing TS6 with LADON+VEIL (`orochi/docs/BRIEF.md:3-12`). The brief explicitly asks for invented technologies, aggressive `comptime`, pinned Zig, x86-64/ARM64 cross-compilation, and constant-time crypto by construction (`orochi/docs/BRIEF.md:14-30`, `orochi/docs/BRIEF.md:45-54`).

Ophion’s current build gives the baseline to improve: 64-bit enforcement, hardening flags, sanitizers, io_uring probing, opssl session export, kTLS detection, subprojects, tests, fuzz targets, and shim upgrade summary (`ophion/meson.build:23-67`, `ophion/meson.build:155-179`, `ophion/meson.build:236-255`, `ophion/meson.build:262-353`, `ophion/meson.build:682-736`). libop already probes event/syscall features and kTLS/session-export defines (`libop/meson.build:45-87`, `libop/meson.build:119-128`). opssl already contains kTLS, PQ toggles, CPU feature probing, TLS/crypto sources, and a no-LTO constant-time AES S-box workaround (`opssl/meson.build:62-98`, `opssl/meson.build:170-232`, `opssl/meson.build:265-288`).

LADON is already a binary mesh concept with 8-byte frames, MessagePack payloads, SWIM membership, CRDT anti-entropy, and VEIL (`ophion/docs/protocols/ladon/overview.md:6-14`). Its wire spec separates native LADON from MessagePack text-link framing and defines HELLO/AUTH/AUTH_OK over Ed25519/X25519 (`ophion/docs/protocols/ladon/wire-protocol.md:6-9`, `ophion/docs/protocols/ladon/wire-protocol.md:47-66`). VEIL wraps post-auth LADON frames with X25519, HKDF-SHA-256, ChaCha20-Poly1305, skipped-key recovery, and serializable ratchet state (`ophion/docs/protocols/veil-security.md:6-8`, `ophion/docs/protocols/veil-security.md:85-104`).

## Invention

### 1. Ringlane I/O

Problem: IRC daemons still spend too much complexity on “which fd is armed, which buffer owns this read, which write queue is safe after migration.” Ophion already probes io_uring and has per-shard event tests (`ophion/meson.build:236-255`, `ophion/tests/meson.build:44-99`).
...
Problem: voice/video users behind hard NATs need relay paths, but the IRC daemon should not become a TURN clone bolted on sideways. Ophion’s media already spans voice, video, negotiation, stats, ABR, and simulcast frames (`ophion/docs/protocols/ladon/media.md:105-124`).
...
Problem: protocol state machines are currently encoded as scattered booleans and comments. LADON has explicit states and pre-established gating rules (`ophion/docs/protocols/ladon/transport-state.md:10-32`).
...
Problem: “constant-time by convention” is not enough for a native crypto rewrite. The brief requires no secret-dependent branches or memory access (`orochi/docs/BRIEF.md:45-54`).
...
Problem: mesh CRDTs, SWIM, ratchets, mode repair, netsplits, and live upgrade cannot be validated well with only black-box scripts. Ophion already tests private spawned daemons and LADON fake peers in Python (`ophion/tests/s2s_helpers.py:470-476`, `ophion/tests/s2s_helpers.py:852-910`).
...
Problem: Ophion’s shim path is powerful but externalized. It transfers plain/kTLS FDs, uses opssl session export for non-kTLS TLS, serializes clients, servers, channel state, media blobs, LADON, VEIL, MessagePack state, and raw send queues (`ophion/ircd/restart.c:135-151`, `ophion/ircd/restart.c:174-190`, `ophion/ircd/restart.c:197-265`, `ophion/ircd/session_migrate.c:728-757`, `ophion/ircd/session_migrate.c:2916-3064`).
...
Problem: IRC daemons grow side-channel callbacks. IRCX already has oper event subscriptions (`ophion/docs/protocols/ircx.md:139-159`).
...
`build.zig.zon` pins `minimum_zig_version` and dependency hashes. Because Zig is pre-1.0, compiler upgrades are explicit “toolchain PRs” with full simulator and conformance runs, matching the brief’s pinning requirement (`orochi/docs/BRIEF.md:48-49`).
...
Keep Ophion’s useful shape: self-contained C tests and pytest private-daemon tests exist today (`ophion/tests/meson.build:1-12`, `ophion/tests/meson.build:106-134`), fuzz targets are opt-in libFuzzer binaries (`ophion/fuzz/meson.build:25-47`), and upgrade tests cover startup/drain/kTLS/S2S survival (`ophion/tests/test_upgrade.py:5-34`, `ophion/tests/test_ktls_upgrade.py:5-26`, `ophion/tests/test_s2s_upgrade.py:5-34`).
