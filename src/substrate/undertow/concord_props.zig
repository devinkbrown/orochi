...
//! Deterministic CRDT property tests for the Concord dotted state types.
...
fn randomContext(allocator: std.mem.Allocator, random: std.Random, dots: usize) !concord.CausalContext {
    var context = concord.CausalContext.init(allocator);
...
fn expectCausalContextLaws(
    a: concord.CausalContext,
    b: concord.CausalContext,
    c: concord.CausalContext,
) !void {
    var ab = try a.clone();
    defer ab.deinit();
    try ab.merge(b);
    ...
    defer ba.deinit();
    try ba.merge(a);
    try expectStateEqual(concord.CausalContext, ab, ba);
    ...
    defer right.deinit();
    try right.merge(bc);
    try expectStateEqual(concord.CausalContext, left, right);

    var idem = try a.clone();
    defer idem.deinit();
    try idem.merge(a);
    try expectStateEqual(concord.CausalContext, idem, a);
}
...
test "lww register randomized merge laws" {
    const Reg = concord.LwwRegister(u16);
