...
meters, or forwards it. This document is the **client contract** Nexus/Ocean must
...
- Server config `[media].native_media_require_mac` (default `false`).
  - `false` — untagged datagrams are accepted (back-compat); a valid tagged
    datagram is also accepted and forwarded.
