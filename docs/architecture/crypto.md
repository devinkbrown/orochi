...
| Do not claim `signature_algorithms_cert` (RFC 8446 §4.2.3) is parsed or enforced. | The extension enum in `tls_extension.zig` models `signature_algorithms` (13) but not `signature_algorithms_cert` (50); no server or client path splits certificate-chain signature policy from CertificateVerify policy. Being closed in a parallel change; unverifiable here. | `src/proto/tls_extension.zig:44`, `src/proto/tls_extension.zig:48` |
