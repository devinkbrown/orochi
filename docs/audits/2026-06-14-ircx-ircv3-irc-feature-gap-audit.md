...
- **Executive Summary highest-risk (1-8)** — all resolved: (1) same-account same-nick is live (`nickHeldBySameAccount` + test; the `if (false)` is gone); (2) MeshPass responder verification landed (`bcb61e6`); (3) remote relay policy — sibling fanout on relayed DMs, `+R`/SILENCE honored, policy-mask protects local `+n/+t/+O/+A`, secured SQUIT (P2); (4) IRCX opt-in `421` gate is live (`needs_ircx`); (5) CAP/ISUPPORT — `CAP LIST`, value-aware `CAP REQ`, backend-gated SASL advertise, ELIST advertised; (6) services/oper security (GHOST ownership, ACCOUNTSET lockdown, fail-closed oper bindings, per-oper REHASH/named-priv gates); (7) IRCX ACCESS enforced at JOIN + DATA via `channelSpeechGate` + EVENT category/priv gates; (8) pure-module wiring is largely intentional (see below).
- **Immediate Runtime Bugs** — #1 same-account-nick, #3 `SessionStore.sessions()` leak (caller-buffer fix), #4 remote-DM sibling fanout all verified fixed. #2 `SESSION RESUME` lightweight-state and #5 join-time bouncer replay are by-design (session reclaim/migration is handled by the upgrade/session-sync path, not a heavyweight resume).
- **Core IRC Gaps** — verified fixed: `+i` honored by WHO/WHOX/NAMES/WHOWAS/LUSERS (`userHiddenByInvisible`); `+g`/ACCEPT 716/717/718; WHO/WHOX double-CRLF; WHOIS secret-channel filter; CTCP auto-reply (`autoReplyCtcp` + test); user `+C` DM enforcement (+ test); channel `+C`; `bot` tag; `$m`/`$z` extbans; LIST/LISTX real-data + 816 (this run).
- **IRCv3 Gaps** — verified fixed: CAP LIST/value-REQ/cap-notify static set; SASL 905/907/908 fidelity + backend gate; `metadata-2` cap-gated handler (+ tests); `labeled-response`; MARKREAD cap-gated; CHATHISTORY membership/LATEST/BETWEEN/batch-type; `message-tags`/TAGMSG validation; EDIT/REDACT/`+draft/typing`/`+draft/unreact` live paths. event-playback fully landed (`617b500`…`e213f35`).
- **IRCX Gaps** — verified fixed: discovery opt-in `421`; DATA via `channelSpeechGate` + STATUSMSG; EVENT unknown-category reject + `event_subscribe` priv gate + SERVER_LINK mirror + cap-gated tags; ACCESS DENY(474)/GRANT-on-JOIN; PROP GET/SET/CLEAR + secret-key join grant + computed providers (MEMBER_OF, ACCOUNT this run); MODEX PRIVATE/HIDDEN routing. **Fixed this run:** MODEX `OPMODERATE` (`U`) query row added now that `U` is enforced (`dc69cfb`) — no longer advertise-without-enforce.
- **S2S / Mesh** — **`[mesh].require_secured` LANDED** (this run): a config gate that refuses plaintext S2S entirely — rejects inbound plaintext peers (handleAccept, before slot alloc) and never dials plaintext outbound (`initiateS2sConnectToAddr` → `error.SecuredS2sRequired`), both gated on `require_secured and !s2sSecured()`. It's a pure policy gate (no wire-format change), so unlike the other mesh items it carried no mesh-flap risk; default false preserves the plaintext fallback. Still genuinely-open, deliberately deferred (wire/consistency risk): per-frame **origin signatures**, full **remote-policy enforcement** (param-mode/quiet-list burst, STATUSMSG-over-S2S, remote `+t`/aggregate-mode authority). SQUIT-secured-links + relay spoof-guard + `+Z`/`+a` slices already landed.
- **Services / Oper** — verified fixed: GHOST ownership, ACCOUNTSET lockdown, REGISTER `*`/VERIFY `<account> <code>`, SASLINFO EXTERNAL, CHANNEL ACCESS/AKICK (AKICK unified this run), USERIP oper-gate, REHASH per-oper privs, named-priv gates. Intentional non-commands: NickServ/ChanServ pseudo-clients (services are real commands), KLINE/DLINE/GLINE/XLINE→WARD, legacy IRCX AUTH/GateKeeper, WATCH (MONITOR is the live path).
- **Config / Security Interop** — verified fixed: `[media].enabled` enforced, `disabled_features`, ISUPPORT boot fail-fast. Open by design/decision: ACME daemon-side renew/hot-reload (CLI/out-of-band), PROXY pre-IRC consumption, news/geo `news_insecure_tls` default, ACME IPv6 nameservers, the `tls.zig` general-client X.509 verifier TODO (other TLS paths do their own chain/CertFP checks). STARTTLS/WEBIRC/ident are intentional exclusions.
- **Documentation Drift** — corrected this run: `_index.md` GROUP/CERTLIST/CERTDEL already right; LISTX/PROP/ACCESS planning marks updated (doc 21); CHANMODES/PREFIX tokens synced (`...WOAVU` + `(YQqov)*!.@+`) across modes.md/isupport.md/architecture; CONFORMANCE.md EVENT/CREATE divergences settled as deliberate; caps.md gained the `draft/event-playback` row + `custom-account-name` 302 value. Residual: minor `numerics.md` undercount (484/485 undocumented but rarely emitted) — cosmetic.
- **Pure Modules Not Wired / Test Coverage** — the not-wired list is mostly intentional (modern-only / WARD / real-command-services stance); test-coverage gaps are additive follow-ups, not defects.

Net: no genuinely-open, tractable, non-risky **code** defect remained beyond the MODEX `OPMODERATE`/`FREETARGET`/`DISFORWARD` rows fixed here (a later check found `F`/`D` are already fully enforced on the `+f` forward path — set-side target check and apply-side `+D` skip — so they were advertised in CHANMODES + MODEX too, not deferred). What's left is the deliberately-deferred risky S2S/mesh wire work, two settled IRCX design divergences (EVENT 806-810 native taxonomy, OID-via-PROP), intentional exclusions, and a cosmetic numerics-doc undercount. Full suite green.
...
LISTX completeness — no more hardcoded blanks (`4344bd4`): the LISTX handler was emitting `created_ms = 0` / `topic_ms = null` and never populating subject/language/registered, while passing `now_ms = 0` to the matcher — so every time-based (`C`/`T`) filter was meaningless and `S=`/`L=`/`R=` matched only empty data. The `listx.zig` engine was already complete (all filters + the 816 truncation writer); the entire gap was handler wiring. Fixed: (1) extended `world.ChannelView` with `created_unix`/`topic_time`/`registered` (real fields already on `Channel`); (2) the handler now builds `ChannelInfo` from real data — `created_ms`/`topic_ms` computed in wall-clock ms from the channel's unix-second timestamps (matching `now_ms = realtimeMillis()`, since `created_unix`/`topic_time` are wall-clock seconds while `nowMs()` is monotonic), `subject`/`language` read from the IRCX PROP store per channel, `registered` from the `+r` ext flag; (3) added a `listx_max_results = 1024` cap that emits `816 RPL_LISTXTRUNC` when exceeded (bounds pathological replies). Every draft LISTX filter now matches real channel data. Gated by a live test (`S=zig*` matches a channel whose SUBJECT prop was set, excludes one without). Full suite green.

AKICK enforcement fixed + command surface unified (`9d32259`): the quarantined "CHANNEL AKICK add mirrors to live join gate" test now passes, and AKICK is a single command. Two findings: (1) the quarantine TODO was **misdiagnosed** — there was no `svc_akick` mask/case bug. The matcher (`globMatch`, case-insensitive) and the services→`chan_akick` mirror were correct; the join gate (`joinDenied`) is only consulted at `joinOne` when `self.world.channelExists(channel)`. A registered channel whose AKICK was added while it had no live presence got *re-created* by the masked joiner ("creating a fresh channel bypasses all gates"), so AKICK never ran. Fixed by extracting `akickDenied()` and consulting it on the create path too (before `world.join`), so a registered channel's AKICK bites even when re-created empty. (2) **Command unification** (per user): there were two AKICK surfaces — the live, services-backed `CHANNEL AKICK` (persistent + join-gate mirror + ACL) and a redundant top-level `AKICK` (registered in `services_ext.zig`) that wrote ONLY to the in-memory `chan_akick` (ephemeral, no persistence, channel-op auth) — a divergent second store. Removed the top-level `AKICK` command (thunk + registration in `services_ext.zig`, the `handleAkick` handler, and a stale orphaned ACCESS doc-comment stacked above it). AKICK is now managed solely via `CHANNEL AKICK`. No test sent the bare command; docs updated (`channels.md`, `_index.md`, `02-world-dispatch-modules.md`). Full suite green.
...
Quarantined tests (skip + TODO, code shipped): MONITOR >64-watcher fanout (66-conn harness limit). ~~CHANNEL AKICK join-gate enforcement~~ — RESOLVED (see top entry): the TODO misdiagnosed the cause. It was never a `svc_akick` mask/case bug (the matcher and the services→`chan_akick` mirror were correct all along); the join gate simply lives in `joinDenied`, which only runs when the channel already exists in `world`. A registered channel whose AKICK was added while it was empty got *re-created* by the masked joiner, bypassing every gate. Fixed by consulting AKICK on the create path too; test un-quarantined and passing.
...
- HIGH-risk mesh items (origin signatures) — wire/handshake changes, mesh-flap risk. (MeshPass responder check landed `bcb61e6`; plaintext-S2S `require_secured` landed in the verification-sweep run — both turned out to be contained, not the feared wire changes.)
...
- ~~`LISTX` is live but incomplete: no query limit/truncation `816`, missing created/topic/subject/language/registered metadata, and filters can be misleading.~~ RESOLVED (see top entry): the handler now populates `ChannelInfo` from real channel + PROP data, computes `C`/`T` ages in wall-clock ms, and caps the reply with `816 RPL_LISTXTRUNC` (`listx_max_results = 1024`). Every draft filter matches real data.
...
- Built-in/computed PROP support covers channel NAME/OID/CREATION/MEMBERCOUNT/MEMBERLIMIT and user MEMBER_OF + ACCOUNT (the latter two via `userBuiltinGet`). Remaining provider keys (`onjoin`/`onpart`/`ownerkey`/`hostkey`) are stored props with proper access tiers; `user_profile` is deferred (no canonical source) (`src/proto/ircx_prop_store.zig:102`).
...
- ~~`OPMODERATE +O` behavior exists but is effectively unreachable: raw `+O` is core `oper_only`, and MODEX lacks OPMODERATE name.~~ RESOLVED: OPMODERATE was reassigned `O`→`U` and made enforceable (`dc69cfb`), and the MODEX `OPMODERATE` (`U`) query row was added (2026-06-14 verification sweep).
- ~~MODEX query omits `F/D` (FREETARGET/DISFORWARD) names.~~ RESOLVED: `F`/`D` turned out to be already fully enforced on the `+f` forward path (set-side: target must be `+F` or setter holds ops there; apply-side: a `+D` target is skipped). They were only unadvertised — now added to both CHANMODES and the MODEX `mode_table` (2026-06-14). `O` is no longer relevant (OPMODERATE moved to `U`).
