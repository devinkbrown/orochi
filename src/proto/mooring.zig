...
//! MOORING ratchet transport state.
//!
//! This module implements the protocol-level symmetric ratchet used after the
//! UNDERTOW/kx layer has already authenticated the peer and produced a shared
...
pub const SealOptions = struct {
    ...
    frame_kind: frame.FrameType = .mooring_ratchet,
...
/// Metadata for an encrypted MOORING_RATCHET payload. `ciphertext` lives in the
...
/// Active MOORING symmetric ratchet. Key material is `Secret([32]u8)` through the
...
pub const State = struct {
    ...
    /// Initialize MOORING from the root secret supplied by the kx layer. This is a
    ...
    /// Encrypt one complete inner UNDERTOW wire frame into `out`.
    pub fn seal(
        self: *State,
        options: SealOptions,
        inner_undertow_frame: []const u8,
        out: []u8,
    ) Error!SealedFrame {
        if (out.len < inner_undertow_frame.len) return error.BufferTooSmall;
        ...
        const ciphertext = out[0..inner_undertow_frame.len];
        const tag = sealAead(&msg_key, nonce, aad, inner_undertow_frame, ciphertext);
    ...
    /// Decrypt one encrypted MOORING frame. Receive-chain state and skipped-key
...
test "in-order seal/open round-trip" {
    ...
    const inner = "complete inner UNDERTOW frame bytes";
...
test "forward gap larger than MOORING_MAX_SKIP is rejected without commit" {
    const allocator = testing.allocator;
    var pair = try makePair();
    defer pair.initiator.deinit();
    defer pair.responder.deinit();
...
test "MOORING key fields use canonical inline-array Secret" {
