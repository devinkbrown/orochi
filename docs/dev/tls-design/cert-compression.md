...
The bomb guard lives on the **decompressor**. In the server-send-only slice, orochi never decompresses (peers/browsers enforce their own guard), so shipping send-only adds a *compressor* with **zero untrusted-input parsing**. The guard becomes load-bearing the moment orochi *offers* compression as a client or accepts compressed *client* certs under mTLS. Design it now:
...
- **Phase ordering minimizes risk:** ship **server-send-only** first (compressor only, no untrusted decode in orochi); add the **client-offer + decompress + bomb guard** as a second increment.
