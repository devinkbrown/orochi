//! Mizuchi structured announcement subsystem.
//!
...
test "publish and active filter by scope and global" {
    ...
    _ = try board.publish("#mizuchi", "ops", "Channel notice", "channel only", "oper", .high, now, 0);
    _ = try board.publish("#other", "ops", "Other channel", "not for us", "oper", .low, now, 0);

    var buf: [16]*const Announcement = undefined;

    // #mizuchi sees its own + the global one.
    const n_chan = board.active("#mizuchi", now, &buf);
    try std.testing.expectEqual(@as(usize, 2), n_chan);
    // Newest-first: #mizuchi was published after the global one.
