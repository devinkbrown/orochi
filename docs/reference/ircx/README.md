# IRCX reference + Mizuchi conformance cross-check

Authoritative IRCX material for Mizuchi:
...
- `ircx-protocol-ophion.md` + `m_ircx_*.md` — ophion's implementation notes
  (behavioral blueprint; NOT authoritative on identity/SID — Mizuchi is SID-free).
- `../ircv3.md` (Mizuchi) + `../ircv3-ophion.md` (richer ophion IRCv3 cap list).

## Cross-check: Mizuchi live IRCX vs the draft (findings)

Mizuchi already implements live: CREATE, ACCESS (801–805), PROP (818/819), EVENT
...
| `RPL_MODEXLIST/END = 806/807` | **806/807 = IRCRPL_EVENTADD/DEL** | **COLLISION.** MODEX is a Mizuchi extension; pick numerics outside 800–819 (draft-reserved) — e.g. a private 8xx range clear of ACCESS/EVENT/LISTX/PROP, and document it. |
...
- **800 IRCRPL_IRCX** discovery (`MODE ISIRCX` / `IRCX`): Mizuchi has `ircx_gate`
...
- **EVENT** numerics 806–810 + event types CHANNEL/MEMBER/SERVER/CONNECTION/
  SOCKET/USER. Mizuchi's EVENT currently rides the daemon Event-Spine categories;
  reconcile names/numerics with the draft (or document the deliberate divergence —
  WALLOPS-as-Event-Spine is an intentional Mizuchi change).
- **AUTH ordering** (AUTH before USER/NICK) — Mizuchi uses IRCv3 SASL via CAP;
...
Treat the draft numerics as authoritative for IRCX-namespaced replies; Mizuchi
