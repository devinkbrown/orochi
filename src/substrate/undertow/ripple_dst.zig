...
//! Deterministic Ocean simulation for RIPPLE witnessed failure detection.
//!
//! A multi-node cluster of `ripple.Ripple` detectors is driven over a seeded, virtual
...
//!
//! The load-bearing invariant is RIPPLE's safety property: **no node is ever
...
const NodeId = ripple.NodeId;
const Rng = ripple.Rng;
const Action = ripple.Action;
const State = ripple.State;
...
const Wire = struct {
    ...
    declare: ripple.Declare = undefined,
};
...
/// Seed-replayable RIPPLE cluster driver.
const Cluster = struct {
    allocator: std.mem.Allocator,
    seed: u64,
    cfg: Config,
    ids: std.ArrayList(NodeId) = .empty,
    nodes: std.ArrayList(ripple.Ripple) = .empty,
    ...
    fn init(allocator: std.mem.Allocator, seed: u64, cfg: Config) !Cluster {
        ...
        while (i < cfg.node_count) : (i += 1) {
            const id: NodeId = @intCast(i + 1); // NodeId 0 is invalid in ripple.
            try self.ids.append(allocator, id);
            try self.nodes.append(allocator, try ripple.Ripple.init(allocator, id, saz_cfg));
    ...
    /// The quorum the detector actually enforces. `ripple.Config.sanitized` floors
    ...
    /// `onPingReq` reply relay — is funneled through here, so a future `ripple.zig`
    /// regression that shipped a dead verdict without a witness quorum cannot slip
    /// past on the ping-req path.
    fn noteDeclare(self: *Cluster, from: NodeId, d: ripple.Declare) void {
        switch (d.state) {
            .dead => {
                self.dead_declares += 1;
                if (d.witnesses.len < self.effectiveQuorum()) {
                    std.debug.print(
                        "RIPPLE solo-DEAD seed=0x{x} declarer={d} node={d} witnesses={d} quorum={d}\n",
...
test "Undertow RIPPLE mesh never declares DEAD without a witness quorum under lossy links" {
...
test "Undertow RIPPLE a fully-reachable cluster with no loss never suspects or kills a node" {
    ...
    while (s < 64) : (s += 1) {
        ...
        if (cluster.suspect_declares != 0 or cluster.dead_declares != 0) {
            std.debug.print(
                "RIPPLE false liveness churn seed=0x{x} suspect={d} dead={d}\n",
...
test "Undertow RIPPLE a hard-partitioned node is declared DEAD only on a witness quorum" {
    ...
    while (s < 64) : (s += 1) {
        ...
        for (cluster.ids.items, 0..) |_, observer| {
            ...
            for (cluster.ids.items, 0..) |_, subject| {
                ...
                if (cluster.status(observer, subject) == .dead) {
                    std.debug.print(
                        "RIPPLE false DEAD of reachable node seed=0x{x} observer={d} subject={d}\n",
        ...
        for (cluster.ids.items, 0..) |_, observer| {
            ...
            if (cluster.status(observer, 0) != .dead) {
                std.debug.print(
                    "RIPPLE isolated victim not detected seed=0x{x} observer={d} state={s}\n",
...
test "Undertow RIPPLE partition detection replays byte-for-byte from its seed" {
