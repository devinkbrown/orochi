...
class AuthorityTests(unittest.TestCase):
    ...
    def write_codex_architect(self, sandbox: str) -> None:
        (validator.CODEX / "onyx-agent-architect.toml").write_text(
            'name = "onyx-agent-architect"\n'
    ...
    def write_claude_architect(self, tools: str) -> None:
        (validator.CLAUDE / "onyx-agent-architect.md").write_text(
            "---\n"
            "name: onyx-agent-architect\n"
            "description: Audit only\n"
            f"tools: {tools}\n"
    ...
    def write_codex_server_integrator(self, instructions: str) -> None:
        (validator.CODEX / "onyx-server-integrator.toml").write_text(
    ...
    def write_claude_server_integrator(self, instructions: str) -> None:
        (validator.CLAUDE / "onyx-server-integrator.md").write_text(
            "---\n"
            "name: onyx-server-integrator\n"
            "description: Sole server writer\n"
            "tools: Read, Grep, Glob, Bash, Write, Edit, Skill\n"
    ...
    def write_codex_docs(self, instructions: str) -> None:
        (validator.CODEX / "onyx-docs.toml").write_text(
            'name = "onyx-docs"\n'
    ...
    def write_codex_release_gate(self, sandbox: str) -> None:
        (validator.CODEX / "onyx-release-gate.toml").write_text(
    ...
    def write_claude_docs(self, instructions: str) -> None:
        (validator.CLAUDE / "onyx-docs.md").write_text(
            "---\n"
            "name: onyx-docs\n"
            "description: Docs writer\n"
            "tools: Read, Grep, Glob, Bash, Write, Edit, Skill\n"
    ...
    def test_codex_architect_rejects_workspace_write(self) -> None:
        ...
        self.assertEqual({"onyx-agent-architect"}, validator.validate_codex())
    ...
    def test_codex_release_gate_enforces_no_edit_contract(self) -> None:
        ...
        self.assertEqual({"onyx-release-gate"}, validator.validate_codex())
    ...
    def test_claude_architect_rejects_unexpected_tools(self) -> None:
        ...
        self.assertEqual({"onyx-agent-architect"}, validator.validate_claude(set()))
    ...
    def test_server_integrator_requires_owner_authority(self) -> None:
        ...
        self.assertEqual({"onyx-server-integrator"}, validator.validate_codex())
        ...
        self.assertEqual({"onyx-server-integrator"}, validator.validate_claude(set()))
    ...
    def test_docs_writer_must_exclude_server(self) -> None:
        ...
        self.assertEqual({"onyx-docs"}, validator.validate_codex())
        ...
        self.assertEqual({"onyx-docs"}, validator.validate_claude(set()))
