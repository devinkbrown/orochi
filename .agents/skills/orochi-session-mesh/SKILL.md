---
name: orochi-session-mesh
description: Implement and verify Orochi reusable sessions across the secured mesh. Use for session tokens, resume, multi-attachment, migration, reclaim, replica convergence, sequential Helix upgrades, or any requirement that every client stay connected and participate from any mesh node.
---
...
Never edit `src/daemon/server.zig`. Report the exact seam and hand it to the permanently named `orochi-server-integrator` role.
