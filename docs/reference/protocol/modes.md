...
| `U` | OPMODERATE | No | Routes messages blocked by moderation gates (`+m`/`+M`/`+Z`) to ops instead of rejecting them. (Letter was `O`, reassigned to `U` — `O` is the enum oper-only channel mode above; opmoderate was previously unreachable because the enum `+O` shadowed it.) | `src/proto/chanmode_ext.zig:66`, `src/daemon/server.zig` (channelSpeechGate / opmod_route) |
