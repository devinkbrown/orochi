...
| 1.4 | **Client-side PQ offering** — client never offers X25519MLKEM768 (server-only today). **Low value** (only orochi's own rarely-PQ outbound). **GOTCHA for the implementer:** use the server's RAW-CONCAT combiner (`mlkem_ss‖x25519_ss` → key schedule, tls_server.zig:902), NOT `kx.HybridKx.decapsulate` (that's the TSUMUGI mesh HKDF combiner — wrong for TLS, would silently break PQ handshakes). Also enlarge the client `keyshare_buf` (currently [256] — the hybrid share is 1216B). Add x25519mlkem768=0x11ec to `supported_groups.NamedGroup`. Verify with an orochi-client↔orochi-server PQ integration test. | S–M | Med | `tls_client.zig`, `supported_groups.zig` |
