...
This matches the Orochi brief: clean-slate Zig-native, Ophion/libop/opssl as reference only, full feature surface, and LADON+VEIL replacing TS6 ([BRIEF.md](/home/kain/orochi/docs/BRIEF.md:1), [BRIEF.md](/home/kain/orochi/docs/BRIEF.md:11), [BRIEF.md](/home/kain/orochi/docs/BRIEF.md:47)).

**1. Daemon Lifecycle**

Use TOML externally, but not Ophion’s legacy callback bridge. Ophion currently parses `ircd.toml` through a TOML bridge and special-cases `[modules]` and `modblacklist` ([toml_conf.c](/home/kain/ophion/ircd/toml_conf.c:4), [toml_conf.c](/home/kain/ophion/ircd/toml_conf.c:511), [toml_conf.c](/home/kain/ophion/ircd/toml_conf.c:551)). Orochi should keep TOML because operators know it, but generate a typed config parser from module schemas at comptime. ZON is better for Zig developers, not IRC operators.
...
Ophion’s current `Client`/`LocalUser` packs identity, transport, caps, WebSocket, SASL, queues, S2S/LADON, and auth state together ([client.h](/home/kain/ophion/include/client.h:200), [client.h](/home/kain/ophion/include/client.h:242)). Orochi should keep the data, but split ownership.

`Channel` stores name, topic, modes, member map, hot local-recipient arrays, bans/except/invex/quiet, props, access entries, history cursor, and LADON CRDT metadata. Ophion already maintains member buckets, ban lists, prop/access pointers, mode logs, caches, and locks ([channel.h](/home/kain/ophion/include/channel.h:93)). Orochi should make those structures explicit and testable.
...
| Chathistory, msgid, event playback, read-marker, search | History ring and caps in [m_chathistory.c](/home/kain/ophion/modules/m_chathistory.c:113); msgid state in [m_chathistory.c](/home/kain/ophion/modules/m_chathistory.c:152) | NATIVE-ZIG | L | Back by `OroStore` history column family. |
...
| Services: MemoServ, VHost/HostServ, service sync, account-notify/FNC | Service DB scope in [services_db.c](/home/kain/ophion/ircd/services_db.c:4); service module caps in [m_services.c](/home/kain/ophion/modules/m_services.c:51) | NATIVE-ZIG | L | Model as service domains over `OroStore`. |
...
Ophion uses SQLite for services and can use SQLite/LMDB for bans ([services_db.c](/home/kain/ophion/ircd/services_db.c:4), [bans_db.c](/home/kain/ophion/ircd/bans_db.c:3)). Orochi’s “no C interop” constraint rules out normal SQLite.
