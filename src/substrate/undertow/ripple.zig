...
//! Pure witnessed Ripple failure detector.
//!
...
pub const Ripple = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, self_id: NodeId, cfg: Config) Error!Ripple {
    ...
    pub fn deinit(self: *Ripple) void {
    ...
    pub fn freeActions(self: *Ripple, actions: []Action) void {
        self.allocator.free(actions);
    }

    /// Advance Ripple state and return caller-owned actions.
    pub fn tick(self: *Ripple, now_ms: i64, rng: *Rng) Error![]Action {
    ...
    pub fn onAck(self: *Ripple, from: NodeId) Error!void {
    ...
    pub fn onMembershipDelta(self: *Ripple, delta: MembershipDelta, now_ms: i64) Error!void {
    ...
    pub fn status(self: *const Ripple, node: NodeId) State {
        if (node == self.self_id) return .alive;
        if (self.findMember(node)) |idx| return self.members.items[idx].state;
        return .dead;
    }

    /// Whether Ripple already tracks `node`. Distinguishes a genuinely-dead
    ...
    pub fn isMember(self: *const Ripple, node: NodeId) bool {
        return node == self.self_id or self.findMember(node) != null;
    }

    fn applyDeltaAboutSelf(self: *Ripple, delta: MembershipDelta) Error!void {
        switch (delta.state) {
            .alive => {
    ...
    fn applyAlive(self: *Ripple, node: NodeId, incarnation: Incarnation) Error!void {
    ...
    fn chooseProbeTarget(self: *Ripple, rng: *Rng) ?usize {
        var count: usize = 0;
        for (self.members.items) |m| {
    ...
    fn appendDirtyDeclares(self: *Ripple, actions: *std.ArrayList(Action)) Error!void {
        if (self.self_dirty) {
    ...
    fn declareFor(_: *Ripple, m: Member) Declare {
        return .{
            .node = m.id,
            .state = m.state,
    ...
    fn ensureMember(self: *Ripple, node: NodeId) Error!usize {
    ...
    fn validatePeer(self: *const Ripple, node: NodeId) Error!void {
        if (!validNode(node) or node == self.self_id) return error.InvalidNode;
    }

    fn findMember(self: *const Ripple, node: NodeId) ?usize {
        for (self.members.items, 0..) |m, idx| {
...
test "healthy node stays alive" {
    var det = try Ripple.init(testing.allocator, 1, .{ .period_ms = 100 });
    defer det.deinit();
    var rng = Rng.init(7);
...
test "missed acks transition alive to suspect and request witnesses" {
    var det = try Ripple.init(testing.allocator, 1, .{ .period_ms = 100, .k = 2 });
    defer det.deinit();
    var rng = Rng.init(11);
...
test "quorum of witnesses transitions suspect to dead" {
    var det = try Ripple.init(testing.allocator, 1, .{
        .period_ms = 100,
        .quorum = 2,
