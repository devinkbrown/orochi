...
//! IRC network-state CRDTs for the UNDERTOW mesh.
//!
...
pub const OrSet = concord.OrSet;
pub const LwwRegister = concord.LwwRegister;
...
/// Complete IRC graph for one UNDERTOW replica.
...
test "network state converges when replicas apply operations in different orders" {
    ...
    const chan = try ChannelName.init("#onyx");
