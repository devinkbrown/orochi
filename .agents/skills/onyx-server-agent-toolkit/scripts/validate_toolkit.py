#!/usr/bin/env python3
"""Validate Onyx Server's project-scoped Codex agents, Claude agents, and skills."""
...
REQUIRED_SHARED = {
    "zig-coder",
    "onyx-session",
    "onyx-server-integrator",
    "onyx-server-dst",
    "onyx-reviewer",
    "onyx-release-gate",
    "onyx-server-deploy",
    "onyx-docs",
    "onyx-agent-architect",
}
CODEX_SANDBOX = {
    "onyx-agent-architect": "read-only",
    "onyx-reviewer": "read-only",
    "onyx-server-deploy": "danger-full-access",
    "onyx-docs": "workspace-write",
    "onyx-server-dst": "workspace-write",
    "onyx-release-gate": "read-only",
    "onyx-server-integrator": "workspace-write",
    "onyx-session": "workspace-write",
    "zig-coder": "workspace-write",
}
CODEX_EFFORT = {
    "zig-coder": "high",
    "onyx-release-gate": "high",
    "onyx-docs": "high",
    "onyx-session": "xhigh",
    "onyx-server-integrator": "xhigh",
    "onyx-server-dst": "xhigh",
    "onyx-reviewer": "xhigh",
    "onyx-server-deploy": "xhigh",
    "onyx-agent-architect": "xhigh",
}
CLAUDE_TOOLS = {
    "onyx-agent-architect": {"Read", "Grep", "Glob", "Skill"},
    "onyx-reviewer": {"Read"},
    "onyx-fast-auditor": {"Read"},
    "onyx-integration-reviewer": {"Read"},
    "onyx-security-reviewer": {"Read"},
    "onyx-release-gate": {"Read", "Grep", "Glob", "Bash", "Skill"},
    "onyx-server-deploy-authorized": {"Read", "Grep", "Glob", "Bash", "Skill"},
    "onyx-docs": {"Read", "Grep", "Glob", "Bash", "Write", "Edit", "Skill"},
    "onyx-server-dst-leaf": {"Read", "Grep", "Glob", "Bash", "Write", "Edit", "Skill"},
    "onyx-server-integrator": {"Read", "Grep", "Glob", "Bash", "Write", "Edit", "Skill"},
    "onyx-session": {"Read", "Grep", "Glob", "Bash", "Write", "Edit", "Skill"},
    "zig-coder-leaf": {"Read", "Grep", "Glob", "Bash", "Write", "Edit", "Skill"},
}
CODEX_SERVER_EXCLUDED_WRITERS = {
    name
    for name, sandbox in CODEX_SANDBOX.items()
    if sandbox != "read-only" and name != "onyx-server-integrator"
}
CLAUDE_MODEL_EFFORT = {
    "onyx-fast-auditor": ("claude-sonnet-5", "low"),
    "onyx-integration-reviewer": ("claude-sonnet-5", "medium"),
    "onyx-security-reviewer": ("claude-sonnet-5", "high"),
    "onyx-release-gate": ("claude-sonnet-5", "medium"),
    "onyx-docs": ("claude-sonnet-5", "medium"),
}
...
def validate_codex() -> set[str]:
    ...
    for path in sorted(CODEX.glob("*.toml")):
        ...
        if name == "onyx-server-integrator" and (
        ...
        if name != "onyx-server-integrator" and role == "owner":
            fail(f"Codex agent {name} cannot declare owner server.zig authority")
        for skill in re.findall(r"\$(onyx-[a-z0-9-]+)", data["developer_instructions"]):
...
def validate_claude(skill_names: set[str]) -> set[str]:
    ...
    for path in sorted(CLAUDE.glob("*.md")):
        ...
        if name != "onyx-server-integrator" and is_claude_writer and (
