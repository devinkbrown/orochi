...
| `CHANMODES` | `beIZ,k,lfj,imnstCTNMSgWOA` | Four channel-mode classes: list, param-always, param-on-set, flag. | Static token from `chanmodes_token`. `W`, `O`, and `A` are live flag modes; see `modes.md`. | `src/proto/protocol_inventory.zig:35`, `src/proto/protocol_inventory.zig:57`, `src/daemon/server.zig:4500` |
