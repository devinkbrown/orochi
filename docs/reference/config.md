...
| `raw_public_key` | bool | `false` | `true` or `false` | Enables RFC 7250 server raw-public-key negotiation. When true, a client that offers `server_certificate_type=RawPublicKey` may receive the active leaf's bare SubjectPublicKeyInfo instead of the X.509 chain. Default off keeps X.509 handshakes byte-identical. |
