#!/usr/bin/env python3
"""Select conservative Orochi build gates from changed repository paths."""
...
def select(paths: Iterable[str]) -> tuple[list[str], set[str]]:
    ...
    for path in paths:
        ...
        if contains(path, "s2s", "suimyaku", "mesh", "relay_v2"):
        ...
        if contains(path, "config", "orochi.reference.toml"):
