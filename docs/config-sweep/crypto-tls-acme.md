...
|-----------|------------------|---------------|------------------|-------------------|------|---------|----------|
| src/daemon/config_format.zig / acme_cli.zig | `directory_url` / CLI staging/prod defaults | Let's Encrypt prod for daemon renewal; staging/prod CLI defaults for `acme-issue` | ACME directory endpoint | `acme.directory_url` *(schema-backed for daemon; CLI also keeps staging/prod overrides)* | string | prod | – |
| src/daemon/config_format.zig / acme_cli.zig | `default_ca_bundle` | `/etc/ssl/certs/ca-certificates.crt` | CA trust-anchor bundle path validating the ACME endpoint cert | `acme.ca_bundle_path` *(schema-backed)* | string | (= current) | – |
| src/daemon/config_format.zig / acme_cli.zig | `default_challenge_port` | `14402` | Loopback HTTP-01 port nginx proxies the challenge path to | `acme.challenge_port` *(schema-backed)* | uint(u16) | 14402 | 1..65535 |
| src/daemon/config_format.zig / acme_cli.zig | `default_ca_bundle_max_bytes` | `4 << 20` (4 MiB) | Max bytes read from the CA bundle file | `acme.ca_bundle_max_bytes` *(schema-backed)* | uint | 4194304 | 64KiB..64MiB |
...
| src/daemon/config_format.zig / acme_runner.zig | `default_max_response_bytes` | `256 * 1024` | Max bytes accepted for a single ACME HTTP response | `acme.max_response_bytes` *(schema-backed)* | uint | 262144 | 16KiB..4MiB |
...
| src/daemon/config_format.zig / acme_runner.zig | `default_error_body_preview_bytes` | `512` | Max bytes of RFC 7807 problem body logged on error/debug | `acme.error_body_preview_bytes` *(schema-backed)* | uint | 512 | 0..4096 |
| src/daemon/config_format.zig / acme_runner.zig | `default_max_steps` | `64` | Max ACME state-machine steps before aborting (loop/hang guard) | `acme.max_steps` *(schema-backed)* | uint | 64 | 8..1024 |
| src/daemon/config_format.zig / acme_runner.zig | `default_resolv_conf_max_bytes` | `64 * 1024` | Max bytes read from /etc/resolv.conf | `acme.resolv_conf_max_bytes` *(schema-backed)* | uint | 65536 | 4KiB..1MiB |
| src/daemon/config_format.zig / acme_runner.zig | `default_dns_port` | `53` | UDP port for DNS A-record lookup of ACME endpoint | `acme.dns_port` *(schema-backed)* | uint(u16) | 53 | 1..65535 |
...
| src/daemon/config_format.zig / acme_http01_listener.zig | `default_listen_backlog` | `16` | TCP accept backlog for the challenge listener | `acme.http01_listen_backlog` *(schema-backed)* | uint | 16 | 1..1024 |
| src/daemon/config_format.zig / acme_http01_listener.zig | `default_accept_poll_ms` | `250` ms | RCVTIMEO so the accept loop wakes to re-check the stop flag | `acme.http01_accept_poll` *(schema-backed)* | duration | 250ms | 50ms..5s |
| src/daemon/config_format.zig / acme_http01_listener.zig | `default_conn_read_timeout_sec` | `5` sec | RCVTIMEO on accepted challenge connections (slow-client guard) | `acme.http01_conn_read_timeout` *(schema-backed)* | duration | 5s | 1s..60s |
