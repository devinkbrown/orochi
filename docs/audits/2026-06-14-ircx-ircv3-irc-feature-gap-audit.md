...
Re-assessed from the worktree wave and found ALREADY LIVE on this HEAD (independently landed; the worktree commits were redundant): WHOIS secret/private-channel hiding (`channelHiddenFromWhois`, with its own test); IRCX opt-in `421` gate across the command surface (`needs_ircx`); MODEX PRIVATE/HIDDEN routing (synth `MODE +p/+h` → `setPrivate`/`setHidden`). Still deferred (NOT mechanical): MODEX OPMODERATE `U` / FREETARGET `F` / DISFORWARD `D` query rows (need the MODE engine to actually enforce those letters first, per `mode_rearchitecture.md` — advertising-without-enforcement risk); NICK-change propagation over the S2S mesh (wire change, mesh-flap risk); ISUPPORT `isupport.Builder` chunker (superseded by `protocol_inventory.currentIsupport()`); IRCX ACCESS GRANT-overrides-DENY precedence (intentionally NOT done — deny-wins is kept as the security default; GRANT auto-status on JOIN itself is now live, `a6091f1`); cross-reactor `publishOperEvent` fanout (pre-existing: oper events only reach opers on the publishing reactor). ATTEMPTED 2026-06-14 via a `DeliverMsg.broadcast_category` fabric message + a `drainFabric` broadcast handler (each reactor delivers the plain form to its own category-subscribers on its own thread) + a `publishOperEvent` fan-out to other shards (waking via `reactors[shard].wake`, the canonical path). The MECHANISM was verified — a broadcast sent from shard 0 was received and processed on shard 1's reactor — but it was REVERTED unlanded because (a) this build sandbox's `SO_REUSEPORT` does not distribute test connections across reactors (every client lands on shard 0, so shard 1 always has 0 clients and no deterministic cross-shard test is possible here), and (b) a separate `num_shards=2` anomaly surfaced where an oper's own `EVENT BROADCAST` self-receipt timed out even all-on-shard-0 — needs root-causing. Land this only in an environment where SO_REUSEPORT distributes accepts (or with a shard-pinning test harness). ROOT-CAUSE UPDATE: while building the harness, isolated a deeper PRE-EXISTING multi-shard bug that blocks both this AND its test — under `num_shards>1`, an oper does NOT receive its own `EVENT BROADCAST` even all-on-shard-0 (single-shard works; the existing oper-event tests all run at the default `num_shards=1`, so this path was never covered). Instrumented `publishOperEvent`: the broadcaster IS found and `self.deliver` returns no error (`total=1 subs=1`), yet the bytes never reach its socket. So it is a SEND-FLUSH bug, not a lookup/`rx()` bug: delivering to the connection that is *currently being processed* (mid-command) via `self.deliver`/`enqueueDelivery` enqueues+arms the send but the multi-reactor loop never submits/flushes it (single-reactor does). Fix the multi-reactor self-delivery flush FIRST (scope — broadcaster-only vs all same-shard opers — and the exact submit-timing gap need confirming), then the cross-reactor fan-out (code written, mechanism proven) can land on top. This likely affects every `publishOperEvent` caller (KILL/SERVER_LINK/POLICY/ANNOUNCE) for the triggering oper under sharding.
