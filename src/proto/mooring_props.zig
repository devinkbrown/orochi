...
//! Deterministic property and known-answer tests for the MOORING ratchet.
//!
...
const mooring = @import("mooring.zig");
...
const Pair = struct {
    initiator: mooring.State,
    responder: mooring.State,
...
fn makePair(
    ...
    const initiator_role: mooring.Role = .initiator;
    const responder_role: mooring.Role = .responder;
    return .{
        .initiator = try mooring.State.init(root_secret, initiator_role, initiator_nonce, generation, epoch_seconds),
        .responder = try mooring.State.init(root_secret, responder_role, responder_nonce, generation, epoch_seconds),
    };
}

fn toEncrypted(sealed: mooring.SealedFrame) mooring.EncryptedFrame {
    return .{
...
fn expectOpenError(err: mooring.Error) !void {
    switch (err) {
        error.AuthFailed,
...
fn openOkOrTypedError(
    state: *mooring.State,
    options: mooring.OpenOptions,
    encrypted: mooring.EncryptedFrame,
    out: []u8,
) !void {
...
test "KAT fixed initial seal output and open recovery" {
    ...
    // KAT plaintext preserved verbatim: expected_nonce/tag/ciphertext below are a
    // golden vector computed over these exact 32 bytes; renaming the string would
    // silently diverge the known-answer test (value load-bearing, not a codename).
    const plaintext = "tsumugi fixed known-answer frame";
    const expected_nonce: mooring.Nonce = .{ 34, 34, 34, 34, 34, 34, 34, 34, 0, 0, 0, 0 };
    const expected_tag: mooring.Tag = .{
    ...
    const sealed = try pair.initiator.seal(
        mooring.SealOptions{ .outer_header = &outer },
        plaintext,
        &ciphertext,
    );
    ...
    const opened = try pair.responder.open(
        mooring.OpenOptions{ .outer_header = &outer },
        toEncrypted(sealed),
        &recovered,
    );
...
test "in-order bidirectional seal open recovers plaintext across many frames" {
    ...
    const seal_options = mooring.SealOptions{ .outer_header = &outer };
    const open_options = mooring.OpenOptions{ .outer_header = &outer };
...
test "chain keys advance per frame and frame N fails under frame N plus one key" {
    ...
    var wrong_key_receiver = try mooring.State.init(fixedRoot(0x42), .responder, fixedNonceBase(0xb2), 3, 0);
...
test "out of order frames inside max skip open and beyond max skip fails closed" {
    const outer = testOuterHeader();
    const options = mooring.OpenOptions{ .outer_header = &outer };
    const msg = "skip-window frame";

    var pair = try makePair(fixedRoot(0x53), fixedNonceBase(0xc1), fixedNonceBase(0xd2), 4, 0);
    defer pair.deinit();

    var ciphertexts: [mooring.max_skip + 1][msg.len]u8 = undefined;
    var sealed: [mooring.max_skip + 1]mooring.SealedFrame = undefined;
    var i: usize = 0;
    while (i <= mooring.max_skip) : (i += 1) {
    ...
    const opened_last = try pair.responder.open(options, toEncrypted(sealed[mooring.max_skip]), &out);
    try testing.expectEqualSlices(u8, msg, opened_last.plaintext);
    try testing.expectEqual(@as(u32, @intCast(mooring.max_skip + 1)), pair.responder.recv_counter);
    ...
    const opened_middle = try pair.responder.open(options, toEncrypted(sealed[mooring.max_skip / 2]), &out);
    ...
    var far_sealed: mooring.SealedFrame = undefined;
    i = 0;
    while (i <= mooring.max_skip + 1) : (i += 1) {
...
test "open returns typed errors or valid plaintext for arbitrary attacker frame bytes" {
    ...
    var state = try mooring.State.init(randomRoot(random), .responder, randomNonceBase(random), 0x0100_0001, 10);
    defer state.deinit();
    ...
    while (i < attacker_iterations) : (i += 1) {
        ...
        var nonce: mooring.Nonce = undefined;
        var tag: mooring.Tag = undefined;
        random.bytes(&nonce);
        random.bytes(&tag);
        ...
        const counter = switch (i % 11) {
            ...
            2 => state.recv_counter +| @as(u32, @intCast(mooring.max_skip)),
            3 => state.recv_counter +| @as(u32, @intCast(mooring.max_skip + 1)),
            else => random.int(u32),
        };
        if (i % 5 == 0) {
        ...
        try openOkOrTypedError(
            &state,
            mooring.OpenOptions{ .outer_header = outer_storage[0..outer_len] },
            .{
