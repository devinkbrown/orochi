...
services, WebSocket, CHATHISTORY, voice/video) and replaces the legacy TS6
server-to-server protocol with the native **Suimyaku + Tsumugi** cryptographic mesh.

> **Status: running M1 daemon.** Boots on a real io_uring socket and answers
> `PING` + completes `NICK`/`USER` registration (welcome burst). 30+ Zig modules,
> 192 tests green under Zig 0.16. See [`docs/BRIEF.md`](docs/BRIEF.md) and
