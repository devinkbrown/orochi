...
This document records the planning-phase design for Orochi's Suimyaku server-to-server mesh, Tsumugi ratchet, membership model, and CRDT convergence strategy.

## Naming

Orochi's invented names supersede Ophion's. **Suimyaku** is the S2S CRDT mesh (was LADON), **Tsumugi** is the PQ ratchet (was VEIL), **Sazanami** is gossip (was SWIM), and **Goryu** is the CRDT lib. Below, old names appear only in citations to Ophion's superseded specs (overview.md / wire-protocol.md / veil-security.md); Orochi's own systems use the new names.
