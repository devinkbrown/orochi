...
- Errors: `ERR_NORECIPIENT 411`, `ERR_NOTEXTTOSEND 412`, `ERR_NOSUCHNICK 401`, `ERR_NOSUCHCHANNEL 403`, `ERR_CANNOTSENDTOCHAN 404`, `ERR_TOOMANYTARGETS 407`, `ERR_CHANOPRIVSNEEDED 482`, `ERR_NEEDREGGEDNICK 477`, caller-id `ERR_CANTSENDTOUSER 716` / `RPL_TARGNOTIFY 717`, IRCv3 `FAIL` for invalid UTF-8, content-filter blocks, encrypted-message policy, and CTCP blocks. Durable mesh (`MESSAGE_V2`) admission is best-effort and **local-first**: when admission is transiently busy (spool/journal capacity, OOM), local channel members and local DM recipients still receive the line under ordinary delivery; the sender is **not** given `FAIL PRIVMSG TEMPORARILY_UNAVAILABLE` for that mesh miss (a FAIL would make clients retry into recipients who already saw the message). `FAIL … TEMPORARILY_UNAVAILABLE` remains for incomplete multi-device attachment resolution (partial exact-token fan-out), not for mesh durability alone.
