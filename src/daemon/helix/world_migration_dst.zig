...
test "world migration round-trip recovers every channel and ban entry intact" {
    ...
    // Channel "#mizuchi": 4 members, non-null key, real topic.
    const mizuchi_members = [_]Member{
    ...
    const mizuchi_bans = [_]MaskEntry{
    ...
    const bans = [_]BanCapsule{
        .{ .channel = "#mizuchi", .entries = &mizuchi_bans },
        .{ .channel = "#deep", .entries = &deep_bans },
    };

    const total_ban_entries = mizuchi_bans.len + deep_bans.len; // 6
