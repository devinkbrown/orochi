...
//! `yoroi` — the Yoroi crypto toolkit executable (openssl-parity verbs backed
//! by orochi's pure-Zig substrate). Dispatches `yoroi <cmd> [opts]`, buffers
...
fn topUsage(w: *Writer) Writer.Error!void {
    try w.writeAll(
        \\yoroi — pure-Zig crypto toolkit (openssl-parity, Yoroi substrate)
        \\
        \\usage: yoroi <command> [options]   (yoroi <command> --help for details)
        \\
        \\commands:
...
test "yoroi dispatch routes ciphers and rejects unknown commands" {
...
test "yoroi per-command --help renders without touching inputs" {
    var aw = Writer.Allocating.init(testing.allocator);
    defer aw.deinit();
    try dispatch(testing.allocator, std.testing.io, "x509", &.{"--help"}, &aw.writer);
    try testing.expect(std.mem.indexOf(u8, aw.written(), "usage: yoroi x509") != null);

    aw.clearRetainingCapacity();
    try dispatch(testing.allocator, std.testing.io, "genpkey", &.{"-h"}, &aw.writer);
    try testing.expect(std.mem.indexOf(u8, aw.written(), "usage: yoroi genpkey") != null);
}
