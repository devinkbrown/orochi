...
Net: no genuinely-open, tractable, non-risky **code** defect remained beyond the MODEX `OPMODERATE` row fixed here. What's left is the deliberately-deferred risky S2S/mesh wire work, a few IRCX design-divergence niceties (EVENT 806-810 numerics, OID-on-CREATE, FREETARGET/DISFORWARD query rows pending enforcement), intentional exclusions, and residual doc prose. Full suite green.
...
- MODEX query omits `F/D` (FREETARGET/DISFORWARD) names — still open by design: those letters are not yet enforced by the MODE engine, so a query row would advertise-without-enforce. `O` is no longer relevant (OPMODERATE moved to `U` and is now listed).
