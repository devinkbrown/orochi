...
This document records the wire-format decision and implementation order for channel-scoped remote nick propagation into NAMES/WHO.

The projection engine (`world_projection.mergeMembers`, commit 7983299) and the secured S2S link (`secure_s2s_link.SecureLink`, commit d4a32cd) are done. Remote members still need the missing *data*: the mesh must propagate, per channel, *which remote nicks are present and with what status*.
...
Carry membership as a small, convergent, signed event rather than overloading the channel CRDT with identity. This keeps the CRDT identity-light and the mode-convergence clean. Reuse the machinery already in place:
...
The payload is CoilPack-encoded, so it is canonical + signable.
...
| `channel` | `[]u8` |
| `nick` | `[]u8` |
| `status_bits` | `u4`; reuses `MemberStatus` (founder/owner/op/voice) so prefixes render |
| `origin_node` | `u64` |
| `hlc` | `u64` |
...
Add a parallel `channel_members: StringHashMap(MemberMap)` where `MemberMap = StringHashMap(Member)` and `Member = { node: NodeId, status: u4, hlc: u64 }`.

| API | Behavior |
|---|---|
| `applyMembership(op, chan, nick, node, status, hlc)` | LWW by hlc; allocates/frees owned nick keys; bounded by `cfg.max_nicks` per channel. |
| `channelMembers(chan) -> iterator` | Returns `{ nick, node, status }`. |
| `clearChannel(chan)` / eviction on `removeNode` (peer split) | Drops all members homed on a departed node so a netsplit cleans up remote ghosts. |
