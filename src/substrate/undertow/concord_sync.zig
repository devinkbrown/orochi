...
/// Runtime tuning for the RIBLT anti-entropy sync. Carried by `ConcordSync` and
...
pub const ConcordSync = struct {
    ...
    pub fn init(allocator: Allocator) ConcordSync {
        return initWithConfig(allocator, .{});
    }

    pub fn initWithConfig(allocator: Allocator, cfg: Config) ConcordSync {
        return .{
            .allocator = allocator,
    ...
    pub fn deinit(self: *ConcordSync) void {
    ...
    pub fn addLocalDelta(self: *ConcordSync, signed: SignedDelta) Error!void {
        try self.putVerified(signed);
    }

    pub fn rootHash(self: *const ConcordSync) Hash {
        return self.index.rootHash();
    }

    pub fn peer(self: *const ConcordSync) Peer {
        return .{ .root_hash = self.rootHash() };
    }

    pub fn beginAntiEntropy(self: *ConcordSync, peer_info: Peer) Error!Session {
        return Session.init(self, peer_info.root_hash);
    }

    pub fn applyVerified(self: *ConcordSync, verify_fn: VerifyFn) Error!usize {
    ...
    pub fn dropTombstonesBelow(self: *ConcordSync, frontier: clock.VersionVector) Error!usize {
        var dropped: usize = 0;
    ...
    fn putVerified(self: *ConcordSync, signed: SignedDelta) Error!void {
    ...
    fn putOwned(self: *ConcordSync, owned: OwnedDelta) Error!void {
    ...
    fn putPending(self: *ConcordSync, signed: SignedDelta) Error!void {
    ...
    fn applySignedChannelDelta(self: *ConcordSync, signed: SignedDelta) Error!void {
...
pub const Session = struct {
    ...
    fn init(owner: *ConcordSync, peer_root: Hash) Error!Session {
...
test "overlapping divergent stores converge through bounded pump loop" {
    ...
    var a = ConcordSync.init(allocator);
    defer a.deinit();
    var b = ConcordSync.init(allocator);
    defer b.deinit();
...
test "invalid signed delta is rejected and not applied" {
    ...
    var sync = ConcordSync.init(allocator);
    defer sync.deinit();
...
test "signed channel delta survives a wire crossing and applies on the peer" {
    ...
    var peer = ConcordSync.init(allocator);
    defer peer.deinit();
    ...
    var victim = ConcordSync.init(allocator);
    defer victim.deinit();
