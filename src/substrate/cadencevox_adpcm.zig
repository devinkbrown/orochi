...
//! CadenceVox audio codec spike: IMA ADPCM (16-bit PCM <-> 4-bit).
//!
//! A compact, deterministic, allocation-free voice codec building block for the
//! CadenceVox band: each 16-bit PCM sample is coded to a 4-bit nibble against an
...
//! caller can checkpoint/reset per packet (each CadenceVox frame is independently
