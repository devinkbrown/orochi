...
- **S2S / Mesh** — the genuinely-open items remain, deliberately: per-frame **origin signatures**, **plaintext-S2S `require_secured`**, full **remote-policy enforcement** (param-mode/quiet-list burst, STATUSMSG-over-S2S, remote `+t`/aggregate-mode authority) — all HIGH mesh-flap / wire-change risk, intentionally deferred. SQUIT-secured-links + relay spoof-guard + `+Z`/`+a` slices already landed.
