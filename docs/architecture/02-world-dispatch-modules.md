...
| `CHANMODES=beIZ,k,lfj,imnstCTNMSgWOA` | `src/proto/protocol_inventory.zig:36` |
...
The current source has moved daemon-owned post-registration commands into SerpentRegistry. Any planning text or older request language that describes a live registered if/else command chain should be read as historical unless it points to the small non-command branches in `dispatchRegistered` for multiline, registry, WASM, and prereg fallback. Evidence: `src/daemon/server.zig:3429`, `src/daemon/server.zig:3433`, `src/daemon/server.zig:3471`, `src/daemon/server.zig:3502`.
