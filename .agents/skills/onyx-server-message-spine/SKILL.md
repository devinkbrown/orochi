---
name: onyx-server-message-spine
description: Implement and falsify Onyx Server Event Spine and MESSAGE_V2 exact-once delivery. Use for local authorship, signed relay, replay guards, ACK or retention, deferred authority, mixed-version peers, stable msgid/time, or mesh-wide event convergence.
---
...
Use `$onyx-server-integration` for live daemon wiring and `$onyx-server-zig-verification` for gates.
