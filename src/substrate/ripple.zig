...
//! Witnessed RIPPLE membership state machine (UNDERTOW mesh, planning/04).
//!
...
//!
//! This models the *decisions* of RIPPLE (Das/Gupta/Motivala) -- which probes to
//! emit, when a node ages out of suspicion -- as returned values. The caller
//! (the UNDERTOW peer actor, M-later) is responsible for turning a `Probe` into
//! real PING / PING_REQ frames and feeding observed events back in via the
//! `applyAlive` / `applySuspect` / `applyDead` / `applyLeft` methods.
//!
//! Two RIPPLE correctness mechanisms are implemented:
//!
...
//!
//!   2. **Witnessed extension** (planning/04, "Witnessed RIPPLE"). Suspicion and
...
pub const Config = struct {
    ...
    /// Number of indirect-probe witnesses (the `k` in RIPPLE PING_REQ fanout).
    indirect_probe_count: u8 = 3,
    /// Distinct witnesses required before SUSPECT -> DEAD. A value of 1 is
    /// classic RIPPLE; >1 is the Witnessed extension. Must be >= 1.
    witness_quorum: u8 = 2,
...
/// translates these into real UNDERTOW frames; nothing here performs I/O.
...
/// Witnessed-RIPPLE membership table.
pub const Membership = struct {
    ...
    pub fn applySuspect(
        ...
        if (node == self.self_id) {
            ...
            // the rumor. This is the RIPPLE self-refutation path.
            if (incarnation >= self.self_incarnation) {
